...
form.addEventListener('submit', function (e) {
    ...
    fetch(url).then((response) => {
        ...
        if (weatherMain == 'Clear') {
            ...
            myWeatherContainer.style.backgroundColor = '#FC2E20'
            document.body.style.backgroundImage = "url('Images/summer.jpeg')";
            
        }
        if (weatherMain == 'Clouds') {
            image.src = `./Images/clouds.jpg`
            myWeatherContainer.style.backgroundColor = '#2E8BC0'
            document.body.style.backgroundImage = "url('Images/cloudy1.jpg')";
            
        }
        if (weatherMain == 'Rain') {
            image.src = `./Images/rain.jpg`
            myWeatherContainer.style.backgroundColor = '#0C2D48'
            myWeatherContainer.style.color = 'white'
            searchBtn.style.color = 'white'
            document.body.style.backgroundImage = "url('Images/rainy.avif')";
        }
        if (weatherMain == 'Drizzle') {
            image.src = `./Images/Drizzle.png`
            myWeatherContainer.style.backgroundColor = '#145DA0'
            document.body.style.backgroundImage = "url('Images/drizzle1.jpg')";
            myWeatherContainer.style.color = 'white'
            searchBtn.style.color = 'white'
        }
        if (weatherMain == 'Haze') {
